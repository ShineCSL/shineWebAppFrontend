...
currentAccount: any;
    ...
    loadAll() {
        this.leaveConfigService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<LeaveConfig[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/leave-config'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/leave-config', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: LeaveConfig) {
        return item.id;
    }
    registerChangeInLeaveConfigs() {
        this.eventSubscriber = this.eventManager.subscribe('leaveConfigListModification', (response) => this.loadAll());
    }

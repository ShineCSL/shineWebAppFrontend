...
currentAccount: any;
    ...
    loadAll() {
        this.invoiceConfigService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<InvoiceConfig[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/invoice-config'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: InvoiceConfig) {
        return item.id;
    }
    registerChangeInInvoiceConfigs() {
        this.eventSubscriber = this.eventManager.subscribe('invoiceConfigListModification', (response) => this.loadAll());
    }
